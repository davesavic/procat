...
const retrieveProductHierarchy = `-- name: RetrieveProductHierarchy :many
...
type RetrieveProductHierarchyRow struct {
	...
	CompositionFieldID   pgtype.UUID
	CompositionFieldName pgtype.Text
	Level                int32
}

func (q *Queries) RetrieveProductHierarchy(ctx context.Context, productID pgtype.UUID) ([]RetrieveProductHierarchyRow, error) {
	rows, err := q.db.Query(ctx, retrieveProductHierarchy, productID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []RetrieveProductHierarchyRow
	for rows.Next() {
		var i RetrieveProductHierarchyRow
		if err := rows.Scan(
			&i.ProductID,
			&i.ProductName,
			...
			&i.Value,
			&i.ParentProductID,
			&i.CompositionFieldID,
			&i.CompositionFieldName,
			&i.Level,
		); err != nil {
			return nil, err
		}
